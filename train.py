...
def random_batch_sampler(
    ...
    while True:
        start_tokens = torch.randint(num_tokens - seq_len + 1, size=(batch_size,))
        batch = [tokens[start : start + seq_len] for start in start_tokens]
...
def sequential_batch_sampler(
    ...
    Note: If the last batch is incomplete, which could happen when the number
        ...
        the last batch.
    """
    diff  = batch_size * seq_len
    for batch in range(0, len(tokens), ):
        if batch + (diff) > len(tokens):
            break
        yield tokens[batch : batch + (diff)].reshape(batch_size, seq_len).to(device)
...
def cosine_lr_schedule(
    ...
    def get_lr(t: int) -> float:
        ...
        if t < num_warmup_steps:
            ...
            # Step 1: Calculate the ratio of completed steps to total steps and adjust by pi
            pi_ratio = math.pi * (t - num_warmup_steps) / (num_training_steps - num_warmup_steps)

            # Step 2: Compute cosine, add 1, and divide by 2
            cosine_adjustment = (1 + math.cos(pi_ratio)) / 2

            # Step 3: Calculate the range of learning rates
            lr_range = max_lr - min_lr

            # Step 4: Compute the learning rate
            lr = lr_range * cosine_adjustment + min_lr
...
def compute_language_modeling_loss(
    ...
    logits = logits[:, :-1].contiguous().flatten(0, 1)
